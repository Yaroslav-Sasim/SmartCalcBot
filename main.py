...
# --- Токен и ID админа из переменных окружения ---
...
# --- Инициализация пользователей (локально, можно заменить на DB) ---
users = []
...
def calculate_area(message):
    try:
        ...
        bot.send_message(message.chat.id, f"⬛ Площадь: {a*b:.2f}", reply_markup=main_menu())
    except:
        bot.send_message(message.chat.id, "Введите два числа через пробел!", reply_markup=main_menu())

def calculate_perimeter(message):
    try:
        a, b = map(float, message.text.split())
        bot.send_message(message.chat.id, f"📏 Периметр: {2*(a+b):.2f}", reply_markup=main_menu())
    except:
        bot.send_message(message.chat.id, "Введите два числа через пробел!", reply_markup=main_menu())
...
def calculate_radius(message):
    try:
        ...
        bot.send_message(message.chat.id, f"⚪ Радиус: {r:.2f}", reply_markup=main_menu())
    except:
        bot.send_message(message.chat.id, "Введите число!", reply_markup=main_menu())

# --- Webhook для Render ---
WEBHOOK_URL = f"https://smartcalcbot.onrender.com/{BOT_TOKEN}"
bot.remove_webhook()
